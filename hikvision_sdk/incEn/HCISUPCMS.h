...
typedef struct tagNET_EHOME_DEV_REG_INFO_V12
{
    ...
    BYTE   byFirmwareIdenCode[MAX_FIRMWARE_IDENT_CODE_LEN]; /*device's firmware identification code*/
...
typedef struct tagNET_EHOME_CHANNELPARAM
{
    DWORD  dwChannelID;
    BYTE  byRes[16];
}NET_EHOME_CHANNELPARAM, *LPNET_EHOME_CHANNELPARAM;


typedef struct tagNET_EHOME_VOICETALK_ADDITION_PARAM
{
    NET_EHOME_CHANNELPARAM  struChannelParam[64];
    BYTE  byRes[128];
}NET_EHOME_VOICETALK_ADDITION_PARAM, *LPNET_EHOME_VOICETALK_ADDITION_PARAM;
...
typedef struct tagNET_EHOME_VOICE_TALK_IN
{
    ...
    BYTE  byBroadLevel;
    BYTE  byBroadVolume;
    BYTE  byAudioSamplingRate; //audio sampling rate 0-default, 1-16kHZ, 2-32kHZ, 3-48kHZ, 4-44.1kHZ, 5-8kHZ
    BYTE  byListen;
    BYTE  byRes1[2];
#if (defined(OS_WINDOWS64) || defined(OS_POSIX64))
    NET_EHOME_VOICETALK_ADDITION_PARAM   *pStruAppendParam;   // 指向附加信息NET_VCA_APPEND_INFO的指针
#else
    NET_EHOME_VOICETALK_ADDITION_PARAM   *pStruAppendParam;   // 指向附加信息NET_VCA_APPEND_INFO的指针
    BYTE  byRes2[4];
#endif
    BYTE  byRes[103];//113-8-2=103
...
typedef struct tagNET_EHOME_PREVIEWINFO_IN
{
    ...
    DWORD               dwStreamType;   //Stream Type: 0-Main Stream, 1-Sub Stream, 2-Third Stream, 3-VOICELISTEN, 4-Virtual Stream，5-Fifth Stream, 6-Sixth Stream，7-Seventh Stream, 8-Eighth Stream, 9-Ninth Stream, 10-Tenth Stream
...
typedef struct tagNET_EHOME_PLAYBACK_INFO_IN
{
    ...
    union
    {
        ...
        struct 
        {
            ...
            BYTE    byTimeLapse;            //timelapse playback, 0-no，1-yes
        }struPlayBackbyTime;
    }unionPlayBackMode;
...
typedef struct tagNET_EHOME_SYSCOMPONENT_REDUCED_ADDR
{
    ...
    BYTE    szComponentPkgName[32]; //Component package names, No need to display on the client side
...
#define MAX_LF_SEND_BUF_LEN        4 * 1024 * 1024    
...
typedef enum tagNET_ISUP_CMS_LF_CB_TYPE
{
    ...
    DATETYPE_LINK_INFO = 2,         
    DATATYPE_SESSION_ID = 3,       
    DATETYPE_UPLOAD_RESULT = 4,    
    DATETYPE_DOWNLOAD = 5,         
    DATETYPE_DOWNLOAD_END = 6,      
}NET_ISUP_CMS_LF_CB_TYPE;



#define RESULT_LF_SUCCESS           0 
#define RESULT_LF_UNKNOW_EXCEPTION     1 
#define RESULT_LF_FILE_SIZE_LIMIT       2 
#define RESULT_LF_INVALID_FILE        3 
#define RESULT_LF_DEVICE_RESOURCE_ERROR       4  
#define RESULT_LF_FILE_NOT_FOUND       5 
#define RESULT_LF_RESUME_POSITION_ERROR       6 
#define RESULT_LF_CALLBACK_PARAM_ERROR     7  
#define RESULT_LF_TRANS_CHANNEL_BUSY      8 
#define RESULT_LF_TRANS_CHANNEL_NOT_FOUND     9 
#define RESULT_LF_TRANS_CHANNEL_NOT_ESTABLISH   10 
...
#define INTERACT_BY_BUF  0 

typedef enum tagNET_ISUP_CMS_LF_SET_BUSINESS_PARAM
{
    DATATYPE_URL = 0,                  
    DATATYPE_RESUME_BREAKPOINT_URL = 1,      
